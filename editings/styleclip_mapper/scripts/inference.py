...
def run(test_opts):
    ...
    if opts.fourier_features_transforms_path:
        ...
        transforms = None
    dataset = LatentsDataset(root='/content/drive/MyDrive/celeb/CelebAMask-HQ/CelebA-HQ-img', opts=opts)
    ...
    for input_batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                imgs_ , _ = net.decoder([w] ,None, input_is_latent=True, randomize_noise=False, return_latents=True)
                ...
                res_align_  = net.net.grid_align(torch.cat((res_, img_edit  ), 1))

                # consultation fusion
                conditions_ = net.net.residue(res_align_)

            w_hat = w + 0.1 * net.mapper(w)
            x_hat , _ = net.net.decoder([w_hat] , conditions_, input_is_latent=True, randomize_noise=False, return_latents=True)
            res_ = x -  torch.nn.functional.interpolate(torch.clamp(x_hat, -1., 1.), size=(256,256) , mode='bilinear')

            img_edit = torch.nn.functional.interpolate(torch.clamp(imgs_, -1., 1.), size=(256,256) , mode='bilinear')
            res_align_  = net.net.grid_align(torch.cat((res_, img_edit  ), 1))  

            conditions = net.net.residue(res_align_)    
