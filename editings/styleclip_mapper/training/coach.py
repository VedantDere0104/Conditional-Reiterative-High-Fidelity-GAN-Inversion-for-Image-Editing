...
class Coach:
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                x = batch
                x = x.to(self.device)
                w = get_latents(self.net.net , x)
                with torch.no_grad():
                    imgs_ , _ = self.net.decoder([w] ,None, input_is_latent=True, randomize_noise=False, return_latents=True)
                    ...
                    res_align_  = self.net.net.grid_align(torch.cat((res_, img_edit  ), 1))

                    # consultation fusion
                    conditions_ = self.net.net.residue(res_align_)

                w_hat = w + 0.1 * self.net.mapper(w)
                x_hat , _ = self.net.net.decoder([w_hat] , conditions_, input_is_latent=True, randomize_noise=False, return_latents=True)
                res_ = x -  torch.nn.functional.interpolate(torch.clamp(x_hat, -1., 1.), size=(256,256) , mode='bilinear')

                img_edit = torch.nn.functional.interpolate(torch.clamp(imgs_, -1., 1.), size=(256,256) , mode='bilinear')
                res_align_  = self.net.net.grid_align(torch.cat((res_, img_edit  ), 1))  

                conditions = self.net.net.residue(res_align_)    
                x_hat , _ = self.net.net.decoder([w_hat] , conditions, input_is_latent=True, randomize_noise=False, return_latents=True)                          
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                ...
                loss, cur_loss_dict = self.calc_loss(w.unsqueeze(0), x, w_hat.unsqueeze(0), x_hat)
                x_hat = torch.nn.functional.interpolate(torch.clamp(x_hat, -1., 1.), size=(256,256) , mode='bilinear')
    ...
    def configure_datasets(self):
        ...
        elif self.opts.image_dir :
            train_latents = self.opts.image_dir
        else:
        ...
        if self.opts.latents_test_path and os.path.exists(self.opts.latents_test_path):
            ...
            test_latents = self.opts.image_dir
        ...
        train_dataset_celeba = LatentsDataset(root=train_latents,
                                                opts=self.opts , 
                                                transform=transforms)
        test_dataset_celeba = LatentsDataset(root=test_latents,
                                                opts=self.opts , 
                                                transform=transforms)
