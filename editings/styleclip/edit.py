...
def parse_args():
    ...
    parser.add_argument('--input_image_dir' , type=str , default='./test')
    ...
    parser.add_argument("--stylegan_weights", type=str, default='/content/drive/MyDrive/HFGI_New/HFGI/pretrained/stylegan2-ffhq-config-f.pt')
...
def setup_data_loader(args, opts):
    ...
    images_path = args.input_image_dir if args.input_image_dir is not None else dataset_args['test_source_root']
    ...
    if args.n_images is None:
        args.n_images = len(test_dataset)
...
def run():
    args = parse_args()
    net , opts = setup_model(args.ckpt)
    net.cuda()
    ...
    _ , dataloader = setup_data_loader(args , opts)
    # prepare output directory
    args.output_path = os.path.join(args.exp_dir, "styleclip_edits", f"{args.neutral_text}_to_{args.target_text}")
    os.makedirs(args.output_path, exist_ok=True)
    # edit all images
    for idx , (image , image_name) in enumerate(dataloader):
        if args.n_images is not None and idx >= args.n_images:
            break
        image = image.cuda()
        latent = get_latents(net, image)
        edit_image(net , image , image_name, latent, stylegan_model, global_direction_calculator, args)


def edit_image(net , image , image_name, latent, stylegan_model, global_direction_calculator, args):
    ...
    with torch.no_grad():
        ...
        imgs = source_im
        res = image -  torch.nn.functional.interpolate(torch.clamp(imgs, -1., 1.), size=(256,256) , mode='bilinear')
