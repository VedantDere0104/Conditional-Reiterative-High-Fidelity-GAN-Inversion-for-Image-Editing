...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if not input_is_stylespace:
            ...
            out, out_style = self.conv1(out, latent[0], noise=noise[0], weights_delta=None, input_is_stylespace=input_is_stylespace)
            style_vector.append(out_style)

            skip, out_style = self.to_rgb1(out, latent[1], weights_delta=None, input_is_stylespace=input_is_stylespace)
            style_vector.append(out_style)
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(self.convs[::2], self.convs[1::2], noise[1::2], noise[2::2], self.to_rgbs):
            if not input_is_stylespace:
                ...
                if i==7 and conditions is not None:
                    ...
                    out , out_style2  =  out  * (1 + conditions[0]) + conditions[1]
                out , out_style3 = conv2(out, latent[:, i + 1], noise=noise2)
