...
def main(args):
    ...
    edit_directory_path = os.path.join(args.save_dir)
    ...
    for i , batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            # tic = time.time()
            latent_codes_ = get_latents(net, x)
            ...
            #img_edit, edit_latents = editor.apply_interfacegan(latent_codes_[0].unsqueeze(0).cuda(), edit_direction, factor=args.edit_degree)

            if args.edit_attribute == 'inversion':
                img_edit = imgs_
                edit_latents = latent_codes_[0].unsqueeze(0).to(device)
            elif args.edit_attribute == 'age' or args.edit_attribute == 'smile':
                img_edit, edit_latents = editor.apply_interfacegan(latent_codes_[0].unsqueeze(0).to(device), edit_direction, factor=args.edit_degree)
            else:
                img_edit, edit_latents = editor.apply_ganspace(latent_codes_[0].unsqueeze(0).to(device), ganspace_pca, [edit_direction])

            # align the distortion map
            img_edit = torch.nn.functional.interpolate(torch.clamp(img_edit, -1., 1.), size=(256,256) , mode='bilinear')
            res_align  = net.grid_align(torch.cat((res_, img_edit  ), 1))

            # fusion
            conditions = net.residue(res_align)
            result, edit_latents = net.decoder([edit_latents],conditions, input_is_latent=True, randomize_noise=False, return_latents=True)

            result = torch.nn.functional.interpolate(result, size=(256,256) , mode='bilinear')

            # latent_codes = get_latents(net, result)
            # img_edit, edit_latents = editor.apply_interfacegan(edit_latents[0].unsqueeze(0).cuda(), edit_direction, factor=args.edit_degree)
            # calculate the distortion map
            imgs, _ = net.decoder([edit_latents[0].unsqueeze(0).cuda()],conditions, input_is_latent=True, randomize_noise=False, return_latents=True)
            ...
            result_image, latent_codes = net.decoder([edit_latents],conditions, input_is_latent=True, randomize_noise=False, return_latents=True)
            res = x -  torch.nn.functional.interpolate(torch.clamp(imgs, -1., 1.), size=(256,256) , mode='bilinear')

            # ADA
            img_edit = torch.nn.functional.interpolate(torch.clamp(result_image, -1., 1.), size=(256,256) , mode='bilinear')
            res_align  = net.grid_align(torch.cat((res_, img_edit  ), 1))
            ...
            Image.fromarray(np.array(tensor2im(imgs[0]))).save(saving_dir)            
