...
def main(args):
    ...
    for i , batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            if args.edit_attribute == 'inversion':
                # img_edit = imgs
                edit_latents = latent_codes_[i].unsqueeze(0).to(device)
            elif args.edit_attribute == 'age' or args.edit_attribute == 'smile':
                img_edit, edit_latents = editor.apply_interfacegan(latent_codes_[i].unsqueeze(0).to(device), edit_direction, factor=args.edit_degree)
            else:
                img_edit, edit_latents = editor.apply_ganspace(latent_codes_[i].unsqueeze(0).to(device), ganspace_pca, [edit_direction])
